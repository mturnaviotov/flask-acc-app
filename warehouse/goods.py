...
from .models_warehouse import Good

block = Blueprint('goods', __name__)
...
def get(id):
    safe = escape(id)
    id = uuid.UUID(safe)
    item = Good.query.filter_by(id=id).first()
    if 'Accept' in request.headers and 'application/json' in request.headers['Accept']:
...
def create():
    ...
    newitem = Good(name=escape(data['name']))
    db_session.add(newitem)
    db_session.commit()
    item = Good.query.filter_by(name=escape(data['name'])).first()
    if 'Accept' in request.headers and 'application/json' in request.headers['Accept']:
        return jsonify(item.to_dict())
    elif 'Content-Type' in request.headers and 'application/json' in request.headers['Content-Type']:
        return jsonify(item.to_dict())
...
def update(id):
    safe = escape(id)
    id = uuid.UUID(safe)
    item = Good.query.filter_by(id=id).first()#.to_dict()
    dict = item.to_dict()
